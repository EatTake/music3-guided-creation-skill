...
- 本地直接输入硬限制：`caption` 不超过 12000 个 Unicode 字符，`lyrics` 不超过 20000 个 Unicode 字符；编译器超限即拒绝写出。目标后端若有更严格的字节/token 限制，以后端为准。
- 所有进入 caption 的源字段必须是单行文本，禁止 CR/LF；模板拼接前统一清理首尾空白和句末标点。
- 歌词每行保持可唱短句，标签独占逻辑段落。
